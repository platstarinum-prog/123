    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <main>
        <section className="pt-32 pb-16 md:pt-40 md:pb-24 max-w-7xl mx-auto px-4 sm:px-8">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-500 mb-4">Блог</p>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-gray-900 mb-4 break-words">
            Блог веб-<span className="text-gray-400">розробника</span>
          </h1>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {posts.map((p) => (
              <Link
                key={p.href}
                to={p.href}
                className="group block bg-white border border-gray-200 hover:border-gray-300 rounded-xl p-8 transition-all duration-300 hover:shadow-lg hover:shadow-gray-900/5"
              >
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-700 bg-gray-100 px-3 py-1 rounded">{p.tag}</span>
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">{p.date}</span>
                  <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 ml-auto">{p.read}</span>
                </div>
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 group-hover:text-gray-700 transition-colors break-words">
                  {p.title}
                </h2>
                <p className="text-gray-500 leading-relaxed text-sm sm:text-base break-words">{p.desc}</p>
                <span className="inline-block mt-5 text-gray-900 font-semibold text-sm group-hover:underline">
                  Читати →
                </span>
              </Link>
            ))}
